...
sub Assemble
{
	...
	{
		if ($instructs[$i]{inst} !~ /(\w+);$/ || !exists $labels{$1})
...
sub Extract
{
	...
	FILE: while (my $line = <$in>)
	{
		...
		CTRL: foreach my $ctrl (@ctrl)
		{
			...
			if ((exists($relOffset{$inst->{op}}) || exists($absOffset{$inst->{op}})) && $inst->{inst} =~ /(0x[0-9a-f]+)/)
			{
				my $target = hex($1);
...
sub Preprocess
{
	...
	$file =~ /^<REGISTER_MAPPING>(.*?)^<\/REGISTER_MAPPING>/ms;
	...
	my @schedBlocks = $file =~ /^<SCHEDULE_BLOCK>(.*?)^<\/SCHEDULE_BLOCK>/gms;
...
sub processSassLine
{
	...
	if ($line =~ /^\s+\/\*(?<num>[0-9a-f]+)\*\/\s+$InstRe\s+\/\* (?<code>0x[0-9a-f]+)/o)
	{
		return {
			num     => hex($+{num}),
			pred    => $+{pred},
...
sub processSassCtrlLine
{
	...
	return 0 unless $line =~ /^\s+\/\* (0x[0-9a-f]+)/;
